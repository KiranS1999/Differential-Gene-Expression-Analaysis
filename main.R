#Main script to transform raw TPM gene values, visualise and identify genes
#that are significantly differentially expressed genes in SSc fibroblasts
...
#####TRANSFORM:log TPM values, remove null values#####
...
#Gather counts for all samples into one column(reshape)
...
#Heatmap of Top 20 significant genes (pheatmap)
...
#Unreadable if you plot all twenty genes at once therefore plot 10 at a time

#See significant genes wrt thresholds
#set thresholds
padj.cutoff <- 0.05
lfc.cutoff <- 0.58

threshold <- res$padjusted < padj.cutoff & abs(res$logFC) > lfc.cutoff
#only got two genes when we apply threshold;ENSG00000171246 and ENSG00000153885 (mention in analysis)
